# =========================================================================
# FRAMEWORK SCRIPT POWERSHELL DENGAN EKSPOR OTOMATIS (V5.2)
# =========================================================================
...
while (-not $validInput) {
    ...
    elseif ($inputMethod -eq "2") {
        $useAllUsers = $true
        $validInput = $true
        Write-Host "[OK] Mode: Seluruh Mailbox terpilih." -ForegroundColor Green
    }
    else {
...
Write-Host " Field Kolom       : [UserPrincipalName], [DisplayName], [TotalSizeGB], [ItemCount], [Status]" -ForegroundColor Yellow
Write-Host "==========================================================" -ForegroundColor Yellow
...
if ($useAllUsers) {
    Write-Host "Mengambil seluruh data Mailbox... Mohon tunggu." -ForegroundColor Yellow
...
foreach ($entry in $usersToProcess) {
    ...
    Write-Progress -Activity "Checking Mailbox Storage" -Status "User ${userCount}/${totalUsers}"
    ...
    try {
        ...
        # --- PERBAIKAN LOGIKA TOBYTES ---
        # Mengambil string ukuran (contoh: "1.2 GB (1,288,490,188 bytes)") 
        # dan mengekstrak angka di dalam kurung menggunakan Regex agar aman dari Deserialized error
        $sizeRaw = $stats.TotalItemSize.ToString()
        $sizeInGB = 0

        if ($sizeRaw -match "\((?<bytes>[\d,.]+) bytes\)") {
            $bytesString = $matches['bytes'] -replace '[,.]', ''
            $sizeInGB = [Math]::Round([int64]$bytesString / 1GB, 2)
        }
        # --------------------------------
...
if ($scriptOutput.Count -gt 0) {
    ...
    Write-Host "`nLaporan storage berhasil disimpan di: ${resultsFilePath}" -ForegroundColor Cyan
}